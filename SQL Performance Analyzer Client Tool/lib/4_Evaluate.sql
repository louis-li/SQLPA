...
Declare curRules CURSOR FOR 
	Select Id,[CurrentValue],[Operator],[Threshold],[Description] 
	From Analysis.Rules ORDER BY ID

OPEN curRules

FETCH NEXT FROM curRules INTO @id,@CurrentValue,@Operator,@Threshold,@Description

WHILE @@FETCH_STATUS = 0
BEGIN
	SET @SqlCmd = 'INSERT INTO Analysis.Evaluation SELECT ' + Convert(nvarchar(10), @ID) + ' AS ID, ' + @CurrentValue + ' AS [CurrentValue], ' + @Threshold + ' AS [Threshold],CASE WHEN ' + @CurrentValue + ' ' + @Operator + ' ' + @Threshold + ' THEN ''Pass'' ELSE ''Fail'' END AS Result, ''' + @Description + ''' As [Description]'
